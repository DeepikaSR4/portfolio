...
export const projects = [
  ...
  },
  {
    ...
    description: "A detailed UX case study exploring user journeys, wireframing, and prototyping.",
...
export function ProjectsSection() {
  return (
    <section id="projects" className="py-16 sm:py-20 bg-gradient-to-b from-background to-secondary/5">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {projects.map((project, index) => (
            <Card
              key={index}
              className="group hover:shadow-xl hover:border-secondary/50 transition-all duration-300 border border-border rounded-xl overflow-hidden p-0"
            >
              <img
                src={project.image || "/placeholder.svg"}
                alt={project.title}
                className="w-full h-56 sm:h-64 md:h-72 lg:h-80 object-cover block rounded-t-xl group-hover:scale-105 transition-transform duration-300"
              />
                ...
                <div className="flex gap-3">
                  {project.liveUrl && (
                    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer">
                      <Button
                        size="sm"
                        className="bg-secondary hover:bg-secondary/90 text-secondary-foreground text-sm transition-all duration-300"
                      >
